...
int MXKVStoreSetUpdater(KVStoreHandle handle,
  ...
  */
  std::function<void(int, const NDArray&, NDArray*, int)> updt
  = [updater_temp, updater_handle_temp](int key, const NDArray& recv, NDArray* local, int worker_num) {
    NDArray* recv_copy = new NDArray();
    *recv_copy = recv;
    NDArray* local_copy = new NDArray();
    *local_copy = *local;
    updater_temp(key, recv_copy, local_copy, worker_num, updater_handle_temp);
  };
